...
let mod = {
    ...
    boostAllocation() {
        ...
        let myRooms = _.filter(Game.rooms, {'my': true}),
            ...
            timing = (!roomTrading.boostAllocation && Memory.boostTiming.timeStamp + 10 === Game.time) || Game.time % global.ALLOCATE_COMPOUNDS_INTERVAL === 0;
            ...
            for (let room of myRooms) {
                ...
                Object.keys(global.COMPOUNDS_TO_ALLOCATE).forEach(compound => {
                    ...
                    if (compoundObject.allocate && roomEnabled) {
                        ...
                        let storageLab = function (room, compound) {
                            ...
                            storageLabExist = storageLab(room, compound),
                            ...
                            case 'lab':
                                ...
                                if (!storageLabExist) {
                                    ...
                                    if (labArray.length > 1) {
                                        ...
                                        console.log(`found lab in ${room.name} labId: ${storageLab.id}`);
                                    ...
                                    console.log(`Lab ${storageLabExist.id} is already registered in ${room.name} for ${compound}`);
                                    storageLab = Game.getObjectById(storageLabExist.id);
                                }

                                console.log(`${room.name} ${storageLab.id} ${storageLab.mineralAmount} ${storageLab.mineralType}`);
                                ...
                                if (storageLab !== null && storageLab !== undefined) {
                                    ...
                                    let labObject = data.lab.find(labs => labs.id === storageLabId),
                                        ...
                                        labOrderGranted = roomResources >= 100,
                                        labStoreAmount = Math.min(LAB_MINERAL_CAPACITY, orderAmount);

                                    if (!storageLabExist && labOrderGranted) {
                                        console.log(`registering boostLab in ${room.name}, id: ${storageLabId}`);
                                        room.registerBoostLab(storageLabId);
                                    } else if (!_.isUndefined(currentOrder) && currentOrder.orderRemaining === 0 && storageLab.mineralAmount <= compoundObject.labRefilledAt && !roomOrderGranted && !labOrderGranted) {
                                        console.log(`orderGranted: ${roomOrderGranted} unRegistering boostLab in ${room.name}, id: ${storageLabId}`);
                                        room.unRegisterBoostLab(storageLabId);
                                    }

                                    if (roomOrderNeeded && roomOrderGranted) {
                                        console.log(`${room.name} placeRoomOrder to room ${room.name}, ${orderAmount} ${compound}`);
                                        room.placeRoomOrder(storageLabId, compound, orderAmount);
                                        roomFound = true;
                                    } else if (!roomOrderGranted && roomOrderNeeded)
                                        console.log(`there are not enough resources to roomOrder a lab in ${room.name} empireResources: ${empireResources}`);

                                    if (labOrderNeeded && labOrderGranted) {

                                        room.memory.resources.lab[labIndex].orders = [];
                                        room.memory.resources.lab[labIndex].orders.push(energyOrders);

                                        console.log(`storageLab.mineralAmount: ${storageLab.mineralAmount} compoundObject.labRefilledAt: ${compoundObject.labRefilledAt}`);
                                        ...
                                        console.log(`${room.name} placeOrder to registered LAB ${storageLab.id}, ${labStoreAmount} ${compound}`);

                                        room.placeOrder(storageLabId, compound, labStoreAmount);

                                    } else if (storageLab.mineralType !== compound && storageLab.mineralType !== null && labOrderGranted) {
                                        ...
                                        console.log(`labOrders in ${room.name} is deleted`);
                                        console.log(`${room.name} placeOrder to LAB ${storageLab.id}, ${labStoreAmount} ${compound}`);
                                        room.placeOrder(storageLabId, compound, labStoreAmount);

                                    } else if (!currentOrder && labOrderGranted) {
                                        ...
                                        if (data.lab.length === 3 && data.reactions.orders.length > 0) {
                                            //delete reactions
                                            console.log(`reactions deleted in ${room.name} there are only 3 labs`);
                                            room.memory.resources.reactions.orders = [];
                                        }
                                        room.placeOrder(storageLabId, compound, labStoreAmount);
                                    }
                                } else
                                    console.log(`${room.name} NO storage lab candidates found`);
                            ...
                            case 'storage':
                                if (compound === 'power') {
                                    if (_.isUndefined(room.structures.powerSpawn)) {
                                        console.log(`There is no powerSpawn in ${room.name} ${compound} in not distributed`);
                                        break;
                                    }
                                }
                                if (roomOrderNeeded && roomOrderGranted) {
                                    console.log(`${room.name} placeOrder to STORAGE ${room.storage.id}, ${compoundObject.amount} ${compound}`);
